...
function startGame() {
  if (playerTurn === undefined) {
    if (optionContainer.children.length !== 0) {
      infoDisplay.textContent = "Please deploy all your ships first!";
    } else {
...
function handleClick(e) {
  if (!gameOver) {
    if (e.target.classList.contains("taken")) {
      ...
      infoDisplay.textContent = "You hit a computer's ship!";
    ...
    if (!e.target.classList.contains("taken")) {
      infoDisplay.textContent = "It's a Miss!";
  ...
  if (!gameOver) {
    ...
    setTimeout(() => {
      ...
      if (
        ...
        infoDisplay.textContent = "We've been hit!";
    ...
    setTimeout(() => {
      playerTurn = true;
      turnDisplay.textContent = "Our Turn!";
      infoDisplay.textContent = "Where should we FIRE?";
      const allBoardBlocks = document.querySelectorAll("#computer div");
      allBoardBlocks.forEach((block) =>
        block.addEventListener("click", handleClick)
      );
...
function checkScore(user, userHits, userSunkShips) {
  function checkShip(shipName, shipLength) {
    if (
      ...
      if (user === "player") {
        infoDisplay.textContent = `You sunk their ${shipName}!`;
        playerHits = userHits.filter(
          (storedShipName) => storedShipName !== shipName
        );
      }
      if (user === "computer") {
        infoDisplay.textContent = `They sunk your ${shipName}`;
  ...
  if (playerSunkShips.length === 5) {
    infoDisplay.textContent =
      "You destroyed the computer's whole fleet. You Win!";
    gameOver = true;
  }
  if (computerSunkShips.length === 5) {
    infoDisplay.textContent =
      "Your fleet has been defeated... Refresh the troops (page)!";
    gameOver = true;
  }
}
